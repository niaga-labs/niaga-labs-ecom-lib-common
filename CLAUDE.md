...
- Config: `config/`
- Tests: **14** `*_test.go` files across `auth`, `bizday`, `domain`, `eventsourcing`, `monitoring`, `nats`, `outbox` and
  `response`. Measured 2026-09-17 in the container: **89 top-level + 22 subtests pass, 0 fail, 5 skip** by default
  (NIAGA-302 added `bizday`'s 8; NIAGA-309 added `monitoring`'s 5).
